...
export class CodemirrorComponent implements AfterViewInit, OnDestroy {
	...
	public replace(text: string): void {
		...
		const lines: any[] = this.getLines();
		const beforeText: string = this.instance.doc.getRange({ line: 0, ch: 0 }, {
			line: range.from.line,
			ch: range.from.ch
		})
		const afterText: string = this.instance.doc.getRange({
			line: range.to.line,
			ch: range.to.ch
	...
	public appendQueryExpression(queryExpression: string) {

		if (_.isEmpty(this.getSelectedRange()) === false) {
			this.instance.replaceSelection(queryExpression);
			return;
		}

		this.insert(queryExpression);
	}
